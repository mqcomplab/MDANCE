...
- **Efficient:** Completely avoids :math:`O(N^2)` pairwise distance matrices
...
If you're using a metric other than mean-square deviation (MSD), normalize your dataset using `normalize.py <https://github.com/mqcomplab/MDANCE/blob/main/scripts/inputs/normalize.py>`__.

The trajectory data must be a numpy array with shape :math:`(n_{frames}, n_{features})`.

**Example with included data:**

The alanine dipeptide trajectory is available in the package or can be loaded directly:
...
- `scripts/divine/run_divine.py <https://github.com/mqcomplab/MDANCE/blob/main/scripts/divine/run_divine.py>`_
...
- ``divine_nani_scores.csv``
  ...
  - **CH score** (Calinski-Harabasz): Higher is better (max :math:`\infty`)
  - **DB score** (Davies-Bouldin): Lower is better (min :math:`0`)
...
`Analysis Notebook <https://github.com/mqcomplab/MDANCE/blob/main/scripts/divine/analysis_db.ipynb>`__ contains step-by-step tutorial for analyzing results.
...
Once you've determined the optimal ``k``, use `assign_labels.py <https://github.com/mqcomplab/MDANCE/blob/main/scripts/divine/assign_labels.py>`__ to generate frame-level cluster assignments.
...
`Postprocessing Notebook <../examples/postprocessing.html>`__ contains step-by-step tutorial for extracting frames.
