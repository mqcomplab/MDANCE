...
Each pathway (trajectory) should be prepared as a 2D numpy array with shape :math:`(n_{frames}, n_{features})`. All pathways must have the same number of features (columns).
...
MDANCE includes a sample dataset of 80 alanine dipeptide pathways with dihedral angle features at the ``src/mdance/data/ala_pathways/`` directory. These files (``pathway_dih_0.csv`` through ``pathway_dih_79.csv``) contain phi and psi dihedral angles from multiple MD simulations of alanine dipeptide, making them ideal for testing PRISM.
...
For Molecular Dynamics trajectories, see `Preprocessing Notebook <../examples/preprocessing.html>`__ for step-by-step trajectory preprocessing.
...
- `scripts/prism/run_prism.py <https://github.com/mqcomplab/MDANCE/blob/main/scripts/prism/run_prism.py>`_
...
For MD trajectory post-processing, see `Postprocessing Notebook <../examples/postprocessing.html>`__.
